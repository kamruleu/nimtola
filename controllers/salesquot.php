...
class Salesquot extends Controller{
	...
	function __construct(){
		...
		public function savequot(){
				...
				if($data['xquotnum'] == "")
					$keyfieldval = $this->model->getKeyValue("soquot","xrefquot","QUOT","6");
				else
				...
				'". $data['xqty'] ."','". $date ."','". $data['xrate'] ."','0','". $data['xdisc'] ."','1','BDT','None','None','".$data['zemail']."','None','". $data['xcus'] ."',".$year.",".$month.",'".Session::get('sbranch')."','".Session::get('sbranch')."','".Session::get('sbranch')."','".$itemdetail[0]['xtypestk']."')";
