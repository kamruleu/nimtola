			...
			$table = '<table class="table table-striped table-bordered">';
				$table .='<thead>
						...
						foreach($row as $key=>$val){
							...
							$table .='<tr><td align="right" colspan="6"><strong>Total</strong></td><td align="right"><strong>'.number_format($totalqty,2).'</strong></td><td></td><td align="right"><strong>'.$totaltax.'</strong></td><td align="right"><strong>'.$totaldisc.'</strong></td><td align="right"><strong>'.number_format($total,2).'</strong></td></tr>';
							$net=0;
							if(!empty($row)){
							...
							$table .='<tr><td align="right" colspan="10"><strong>Net Total</strong></td><td align="right"><strong>'.number_format($net,2).'</strong></td></tr>';
							if(!empty($row)){
							$table .='<tr><td align="right" colspan="10"><strong>Received Amt.</strong></td><td align="right"><strong>'.$row[0]["xrcvamt"].'</strong></td></tr>';
							}	
					$table .="</tbody>";			
			$table .= "</table>";
		...
		function confirmpost(){
				...
				if(!empty($glinterface)){
					...
					$month = date('m',strtotime($date)); 
						...
						$data['bizid'] = Session::get('sbizid');
						$data['zemail'] = Session::get('suser');
						$data['xnarration'] = "Created by system! Sales Order:".$sonum.$narretion." Customer: ".$cus."-".$_POST["xorg"];
						$data['xyear'] = $xyear;
						$data['xper'] = $per;
						...
						foreach($glinterface as $key=>$val){
							...
							if($amt<>0)
								...
								'". $amt ."','1','". $amt ."','". $val['xaction'] ."','Truck Fair - ". $trucfair ."'),";
						}
		...
		function cancelpost(){
				...
				if(!empty($glinterface)){
					...
					$month = date('m',strtotime($date)); 
						...
						$data['bizid'] = Session::get('sbizid');
						$data['zemail'] = Session::get('suser');
						$data['xnarration'] = "Created by system! Cancelation of Sales Order:".$sonum.$narretion." canceled by ". Session::get('suser') ." Customer: ".$cus."-".$_POST["xorg"];
						$data['xyear'] = $xyear;
						$data['xper'] = $per;
						$data['xvoucher'] = $voucher;
						$data['xdate'] = $date;
						...
						foreach($glinterface as $key=>$val){
							...
							if($amt<>0)
								$vals .= "(" . Session::get('sbizid') . ",'". $voucher ."','". $i ."','". $val['xacc'] ."',
								'". $acc[0]['xacctype'] ."','". $acc[0]['xaccusage'] ."','". $acc[0]['xaccsource'] ."','". $subacc ."','".Session::get('sbizcur')."',
								'". $amt ."','1','". $amt ."','". $flag ."','Truck Fair - ". $trucfair ."'),";
						}
