...
class Diststocktransfer extends Controller{
	...
	function __construct(){
		...
		foreach($usersessionmenu as $menus){
				if($menus["xsubmenu"]=="Stock Transfer")
					$iscode=1;				
		}
		if($iscode==0)
		...
		public function savedistreq(){
				...
				'". $data['xqty'] ."','". $date ."','". $data['xstdcost'] ."','". Session::get('suser') ."','". $data['xwh'] ."',
				'". $xyear ."','". $month ."','". $data['xtxnwh'] ."','Stock Transfer',-1,'". $itemdetail[0]['xunitstk'] ."','". $itemdetail[0]['xtypestk'] ."')";
		...
		public function edit(){
			...
			$form = new Form();
				...
				$itemdetail = $this->model->getItemDetail("xitemcode", $data['xitemcode']);
		...
		function renderTable($ptnum){
			...
			return $table->myTable($fields, $row, "xrow",URL."diststocktransfer/showitem/".$ptnum."/",URL."diststocktransfer/deleteitem/".$ptnum."/");
