...
class Glinterface extends Controller{
	...
	function __construct(){
		...
		**/
		function renderTable($codetype){
			...
			return $table->createTable($fields, $row, "xsl", URL."glinterface/showcode/".$codetype."/", URL."glinterface/deletecode/".$codetype."/");
		...
		public function showcode($codetp="", $xsl=""){
			...
			foreach ($this->model->getCode($xsl) as $key=>$value){
				$sl = $value['xsl'];
				$xacc = $value['xacc'];
	...
	public function deletecode($codetp, $xsl){
