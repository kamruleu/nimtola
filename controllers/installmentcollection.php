...
class Installmentcollection extends Controller{
	...
	function __construct(){
		...
		public function savecollection(){
				...
				$result = "";
				//echo $cus."-".$year."-".$xdat;
				$getfessdt = $this->model->getsinglefees($cus, $year, $xdat);
				//print_r($getfessdt);die;
				$cus = $getfessdt[0]['xcus'];
				$amount = $getfessdt[0]['xamount'];
				$inssl = $getfessdt[0]['xinssl'];
				$insnum = $getfessdt[0]['xinsnum'];
		...
		public function confirmcollection(){				
				...
				$cur = new Currency();
				$inword=$cur->get_bd_amount_in_text($debamt);
				...
				if($success>0){
					...
					//$smsresult = $sendsms->send_ayub_sms('Dear Customer, Received with thanks from '.$org.' (ID#'.$cus.') for the amount of Tk. '.$debamt.' ('.$inword.') as installment of '.$_POST['xnarration'].'',$mobile);
