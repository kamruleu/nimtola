...
class Glpayvou extends Controller{
	...
	function __construct(){
		...
		foreach($usersessionmenu as $menus){
				if($menus["xsubmenu"]=="Payment Voucher")
					$iscode=1;				
		}
		if($iscode==0)
		...
		public function saveglpayvou(){
				...
				if(isset($_POST["xacc"])){
					$accpay = $_POST["xacc"];
					$payacc = $this->model->getAccDt($accpay);
				}
				//print_r($accdt);die;
				try{
				...
				$data['xacctype'] = $payacc[0]['xacctype'];
				$data['xaccusage'] = $payacc[0]['xaccusage'];
				$data['xaccsource'] = $payacc[0]['xaccsource'];
