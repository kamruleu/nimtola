...
class Glchequeregister extends Controller{
	...
	function __construct(){
		...
		if($rows[0]['xcolsl'] !="" || $rows[0]['xcolsl'] !=NULL){
			$postdatains=array(
			"xstatus" => "Bounced"			
			);
			
			$whereins = " bizid = ". Session::get('sbizid') ." and xcollectionno = ". $rows[0]['xcolsl'];

			$this->model->recupdate("installmentcoll",$postdatains, $whereins);
					}
		$this->chequeentry();
		}
		...
		function confirm($sl=0){
			$rows = $this->model->getChequeRegister($sl);
				...
				$success = $this->model->confirmcheque($data, $cols, $vals);
					...
					if($success>0 && $rows[0]['xcolsl'] !="" || $rows[0]['xcolsl'] !=NULL){
						$postdatains=array(
						"xstatus" => "Confirmed",
						"xvoucher" => $keyfieldval,			
						);
					...
					$this->model->recupdate("glchequereg",$postdata, $where);
						if($rows[0]['xacccrsource'] == 'Customer'){
							$mobile = $this->model->getCustomer($rows[0]['xaccsubcr'])[0]['xphone'];
							$cur = new Currency();
							$inword=$cur->get_bd_amount_in_text($rows[0]['xprime']);
							$sendsms = new Sendsms();

							//$smsresult = $sendsms->send_ayub_sms('Dear Customer, Received with thanks from '.$rows[0]['xaccsubdesccr'].' (ID#'.$rows[0]['xaccsubcr'].') for the amount of Tk. '.$rows[0]['xprime'].' ('.$inword.') as installment of '.$rows[0]['xrefno'].'',$mobile);
							// print_r($smsresult);die;
							// if (strpos($smsresult, 'ERR') !== false) {
							// 	$result = "Failed to send SMS!";
							// }else{
							// 	$result = "SMS send successfully!";
							// }
						}
