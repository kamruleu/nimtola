...
class Purchasegrn extends Controller{
	...
	function __construct(){
		...
		foreach($usersessionmenu as $menus){
				if($menus["xsubmenu"]=="Goods Receive Note")
					$iscode=1;				
		}
		if($iscode==0)
		...
		public function savegrn(){
				...
				$cols = "`bizid`,`xponum`,`xgrnnum`,`xrow`,`xitemcode`,`xitembatch`,`xqtypur`,`xextqty`,`xdate`,`xratepur`,`xwh`,`xbranch`,`xproj`,`xtaxrate`,`xdisc`,`xexch`,`xcur`,`xtaxcode`,`xtaxscope`,`zemail`,`xunitpur`,`xconversion`,`xunitstk`,`xstdcost`,xtypestk";
				
				$vals = "(" . Session::get('sbizid') . ",'". $data['xponum'] ."','". $keyfieldval ."','". $rownum ."','". $data['xitemcode'] ."','". $data['xitemcode'] ."',
				'". $data['xqty'] ."','". $data['xextqty'] ."','". $date ."','". $data['xratepur'] ."','". $data['xwh'] ."','". $data['xwh'] ."','". $data['xwh'] ."','0','0','1','BDT','None','".$item[0]['xunitpur']."','".$data['zemail']."','None','".$item[0]['xconversion']."','".$item[0]['xunitstk']."','".$cost."','".$item[0]['xtypestk']."')";
		...
		public function creategrnall($txnnum){
			...
			$i++;	
				...
				$vals .= "('".$value['zemail']."'," . Session::get('sbizid') . ",'". $keyfieldval ."','". $value['xponum'] ."',". $value['xrow'] .",". $i .",'". $date ."','". $value['xitemcode'] ."','". $value['xitemcode'] ."',
					'". $value['xqty'] ."','0',". $value['xqty'] .",'". $value['xunitstk'] ."',". $rate .",".$cost.",'". $value['xtaxrate'] ."','". $value['xdisc'] ."','". $value['xwh'] ."','". Session::get('sbranch') ."','". Session::get('sbranch') ."','1','". Session::get('sbizcur') ."','". $value['xunitpur'] ."','". $value['xtypestk'] ."','". $value['xconversion'] ."'),";
	...
	public function confirmgrn(){
				...
				if(!empty($glinterface)){
					...
					$narretion = "";
					$rodqty = $this->model->getrodqty($grnnum);
					$cementqty = $this->model->getcementqty($grnnum);
					...
					if(!empty($cementqty))	
						...
						$voucher = $this->model->getKeyValue("glheader","xvoucher","GRNV-","6");
						
						$data = array();

						$data['bizid'] = Session::get('sbizid');
						$data['zemail'] = Session::get('suser');
						$data['xnarration'] = "Total".$narretion." from Supplier: ".$sup."-".$_POST["xsupname"]." and GRN No ".$grnnum;						
						$data['xyear'] = $xyear;
						$data['xper'] = $per;
			...
			$voucher = "";
				...
				if(!empty($glinterface)){
					...
					$month = date('m',strtotime($date)); 
						...
						$voucher = $this->model->getKeyValue("glheader","xvoucher","GCLV-","6");
						
						$data = array();

						$data['bizid'] = Session::get('sbizid');
						$data['zemail'] = Session::get('suser');
						$data['xnarration'] = "Created by system! Cancelation of Goods Receipt Note:".$grnnum." Canceled By ".Session::get('suser')." Supplier: ".$sup."-".$_POST["xsupname"];						
						$data['xyear'] = $xyear;
						$data['xper'] = $per;
						$data['xvoucher'] = $voucher;
						$data['xdate'] = $date;
	...
	public	function renderGrnTable($txnnum){
		...
		function polist(){
			...
			$cols = array("xponum-PO No","xsupdoc-DO","xsup-Supplier","xsupname-Name");
			$table = new Datatable();
		...
		function grnlist($status){
			...
			$cols = array("xdate-Date","xgrnnum-GRN No","xponum-PO No","xsup-Supplier","xsupname-Name","xsupdoc-Suplier Doc");
			$table = new Datatable();
		...
		public function showpost($result=""){
