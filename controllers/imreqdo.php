...
class Imreqdo extends Controller{
	...
	function __construct(){
		...
		public function getsinglereq($sonum,$sl){
			$this->model->getSingleQty($sonum,$sl);
		}
	...
	public	function renderTable($txnnum){
			...
			return $table->myTableSubmit($fields, $row, "xrow", URL."imreqdo/getsinglereq/".$txnnum."/");
	...
	public	function renderDoTable($txnnum){
		...
		function createdelivery($txnnum){
			...
			$i++;	
				...
				'". $value['xbal'] ."','". $date ."','". $value['xrate'] ."','0','". $value['xwh'] ."','". Session::get('sbranch') ."','". Session::get('sbranch') ."','". $value['xtaxrate'] ."','". $value['xdisc'] ."',". $value['xexch'] .",'". $value['xcur'] ."','". $value['xtaxcode'] ."','". $value['xtaxscope'] ."','".$value['zemail']."','". $value['xunitsale'] ."','". $value['xcus'] ."',".$cost.",'". $value['xtypestk'] ."'),";
