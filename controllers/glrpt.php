...
class Glrpt extends Controller{
	...
	function __construct(){
		...
		function glrptmenu(){
			...
			$menuarr = array(
			0 => array("Account Ledger"=>URL."glrpt/ledger","Subsidiary Ledger"=>URL."glrpt/subledger","Customer Ledger"=>URL."glrpt/customerledger","Supplier Ledger"=>URL."glrpt/supplierledger"),
			1 => array("Account Receivable"=>URL."glrpt/incexpledger/Acounts Receivable","Accounts Payable"=>URL."glrpt/incexpledger/Accounts Payable","Expenditure"=>URL."glrpt/incexpledger/Expenditure","Income"=>URL."glrpt/incexpledger/Income"),
			2 => array("Trial Balance"=>URL."glrptstatement/trialbal","Income Statement"=>URL."glrptstatement/plstatementbydate","Balance Sheet"=>URL."glrptstatement/balancesheet","Day Book"=>URL."glrpt/daybook"),			
			3 => array("Customer Balance"=>URL."glrptstatement/shocusbal","Supplier Balance"=>URL."glrptstatement/shosupbal","Receipt Payment"=>URL."glrptstatement/rcptnpayment","Cash Book"=>URL."glrpt/cashbook"),
			4 => array("Bank Book"=>URL."glrpt/bankbook","Customer Type Balance"=>URL."glrpt/custypebal","Salesman Wise Customer Balance"=>URL."glrpt/salesmanwisebal","Salesman Target Collection"=>URL."glrpt/salesmantarcol"),
			);
		...
		public function showcashbook(){
			...
			$this->view->table = $table->cashbankbookchange( $tblvalues, $opbal, "xacc");
		...
		function custypebal(){
			...
			$values = array(
				...
				$this->view->dynform = $dynamicForm->createFourColumnForm($fields, URL."glrpt/shocustypebal", "Show Report",$values);
		...
		function shocustypebal(){
			$this->view->breadcrumb = '<ul class="breadcrumb">
							<li><a href="'.URL.'glrpt/glrptmenu">GL Reports</a></li>
							<li><a href="'.URL.'glrpt/custypebal">Report Filter</a></li>
							<li class="active">Customer Type Balance</li>
						   </ul>';
			$tblsettings = array(
		...
		function salesmanwisebal(){
			$this->view->breadcrumb = '<ul class="breadcrumb">
							<li><a href="'.URL.'glrpt/glrptmenu">GL Reports</a></li>
							<li class="active">Report Filter</li>
						   </ul>';
			$btn = array(
				"Clear" => URL."glrpt/salesmanwisebal",
			);	
			$dt = date("Y/m/d");
			...
			$values = array(
				...
				$this->view->dynform = $dynamicForm->createFourColumnForm($fields, URL."glrpt/shosalesmanwisebal", "Show Report",$values);
		...
		function shosalesmanwisebal(){
			$this->view->breadcrumb = '<ul class="breadcrumb">
							<li><a href="'.URL.'glrpt/glrptmenu">GL Reports</a></li>
							<li><a href="'.URL.'glrpt/salesmanwisebal">Report Filter</a></li>
							<li class="active">Salesman Wise Customer Balance</li>
						   </ul>';
			$tblsettings = array(
				"columns"=>array("0"=>"Customer Code","1"=>"Name",2=>"Address",3=>"Mobile",4=>"Balance"),
				"groupfield"=>"Salesman~xagent",
				"grouprecords"=>array(), //database records columns to show in group
				"detailsection"=>array("xaccsub","xorg","xadd1","xmobile","xbal"),
				//"buttons"=>array("Show"=>URL."test/","Delete"=>URL."test/"),
				//"urlvals"=>array("id","phone"),
			...
			$row = $this->model->salesmanwisebal($xagent, $fdate);
			$totalpoint = "";			
			$this->view->vfdate = $xfdate;
			$this->view->vtdate = $xfdate;
			$this->view->vrptname = "Salesman Wise Customer Balance Report";
			$this->view->org=Session::get('sbizlong');
			$table = new ReportingTable();
			$this->view->table = $table->SingleGroupReportingtbl($tblsettings, $row);
		...
		// 	$this->view->breadcrumb = '<ul class="breadcrumb">
		// 					<li><a href="'.URL.'glrpt/glrptmenu">GL Reports</a></li>
		// 					<li><a href="'.URL.'glrpt/shocustypebal">Customer Type Balance Form</a></li>
		// 					<li class="active">Customer Type Balance</li>
		// 				   </ul>';
		...
		// 	$this->view->renderpage('glrpt/glrptfilter');
		// }
		...
		function salesmantarcol(){
			...
			$btn = array(
				"Clear" => URL."glrpt/salesmantarcol",
			);	
			$dt = date("Y/m/d");
			...
			$values = array(
				...
				$this->view->dynform = $dynamicForm->createFourColumnForm($fields, URL."glrpt/shosalesmantarcol", "Show Report",$values);
		...
		function shosalesmantarcol(){
			$this->view->breadcrumb = '<ul class="breadcrumb">
							<li><a href="'.URL.'glrpt/glrptmenu">GL Reports</a></li>
							<li><a href="'.URL.'glrpt/salesmantarcol">Report Filter</a></li>
							<li class="active">Salesman Target Collection</li>
						   </ul>';
			$tblsettings = array(
				"columns"=>array("0"=>"Item","1"=>"Target Percent (%)","2"=>"Target Qty","3"=>"Sales Qty","4"=>"Unit", "5"=>"Sales Amount"),
				"groupfield"=>"Date~xagent",
				"grouprecords"=>array(), //database records columns to show in group
				"detailsection"=>array("xbrand","xqty","xunitsale","xbal"),
				//"buttons"=>array("Show"=>URL."test/","Delete"=>URL."test/"),
				//"urlvals"=>array("id","phone"),
				"sumfields"=>array("xbal"),
				);
			...
			$this->view->vrptname = "Salesman Target Collection Report </br>".$xagent;
			$this->view->org=Session::get('sbizlong');
			$table = new ReportingTable();
			$this->view->table = $table->SingleGroupReportingtblTar($tblsettings, $row, $saltarbal,$salcolbal, $saltarpre,$saltarrod,$saltarsc,$cusareabal);
		...
		public function showcusledger(){
			...
			$cusdt = $this->model->getcusdt($cus);
			$org = $cusdt[0]['xorg'];
			$add1 = $cusdt[0]['xadd1'];
			$phone = $cusdt[0]['xphone'];
			...
			$this->view->table = $table->CusledgerTable($fields, $tblvalues, "xvoucher", $opbal, $cus, $org, $add1, $phone, "Customer");
