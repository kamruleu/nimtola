...
class Imreturn extends Controller{
			...
			$voucher = "";
				...
				if(!empty($glinterface)){
					...
					$month = date('m',strtotime($date)); 
						...
						$voucher = $this->model->getKeyValue("glheader","xvoucher","DORN-","6");
						
						$data = array();

						$data['bizid'] = Session::get('sbizid');
						$data['zemail'] = Session::get('suser');
						$data['xnarration'] = "Created by system! Delivery Return:".$retunnum." Customer: ".$cus."-".$_POST["xorg"];
						$data['xyear'] = $xyear;
						$data['xper'] = $per;
	...
	public	function renderTable($txnnum){
			...
			return $table->myTableSubmit($fields, $row, "xreqdeldetsl", URL."imreturn/getsinglereq/");
	...
	public	function renderReturnTable($txnnum){
		...
		function reqlist(){
			...
			$cols = array("xreqdelnum-DO No","xdate-Date","xcus-Customer No","xorg-Name");
			$table = new Datatable();
			$this->view->table = $table->createTable($cols, $rows, "xreqdelnum",URL."imreturn/showentry/");
