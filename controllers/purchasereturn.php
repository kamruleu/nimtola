...
class Purchasereturn extends Controller{
	...
	function __construct(){
		...
		public function save(){
				...
				'". $data['xqty'] ."','". $data['xextqty'] ."','". $date ."','". $data['xratepur'] ."','". $data['xratepur'] ."','". $data['xwh'] ."','". $data['xwh'] ."','". $data['xwh'] ."','". $data['xtaxrate'] ."','0','1','BDT','None','".$item[0]['xunitpur']."','".$data['zemail']."','None','".$item[0]['xconversion']."','".$item[0]['xunitstk']."','".$rownum."','".$item[0]['xtypestk']."')";
	...
	public function confirm(){
				...
				if(!empty($glinterface)){
					...
					$month = date('m',strtotime($date)); 
						...
						$voucher = $this->model->getKeyValue("glheader","xvoucher","PRTN-","6");
						
						$data = array();

						$data['bizid'] = Session::get('sbizid');
						$data['zemail'] = Session::get('suser');
						$data['xnarration'] = "Created by system! Purchase Return:".$returnnum." Supplier: ".$sup."-".$_POST["xsupname"];						
						$data['xyear'] = $xyear;
						$data['xper'] = $per;
			...
			$voucher = "";
				...
				if(!empty($glinterface)){
					...
					$month = date('m',strtotime($date)); 
						...
						$data['bizid'] = Session::get('sbizid');
						$data['zemail'] = Session::get('suser');
						$data['xnarration'] = "Created by system! Cancelation Of Purchase Return:".$returnnum." Created By ".Session::get('suser')."Supplier: ".$sup."-".$_POST["xsupname"];						
						$data['xyear'] = $xyear;
						$data['xper'] = $per;
						$data['xvoucher'] = $voucher;
						$data['xdate'] = $date;
