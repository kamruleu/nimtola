...
class Glrptstatement extends Controller{
				...
				$table = '<table class="table" border="1" width="100%">';
					...
					$table .= '<tr>';
						$table .= '<td>';
							...
							$table .= '</tr>';
							$table .= '<tr>';
								$table .= '<td><strong>Cash In Hand :'.number_format($cashopbal, $this->decimals, '.', '').'</strong></td>';
							$table .= '</tr>';
							$table .= '<tr>';
							$table .= '<td><strong>Cash At Bank :'.$bankopbal.'</strong><td></td><td><strong>'.number_format($totalop, $this->decimals, '.', '').'</strong></td>';
							$table .= '</tr>';
								...
								if(!empty($tblvaluesrcptpay)){
									foreach($tblvaluesrcptpay as $key=>$value){
										if($value["xprime"]<0){
											...
											$rcpt += $value["xprime"];
											$table .= '<tr>';
												$table .= '<td>'.$value["xacc"].'</td><td>'.$value["xaccdesc"].'</td><td>'.number_format(abs($value["xprime"]), $this->decimals, '.', '').'</td>';
											$table .= '</tr>';
										}
									}
						...
						$table .= '<td>';
								...
								if(!empty($tblvaluesrcptpay)){
									foreach($tblvaluesrcptpay as $key=>$value){
										if($value["xprime"]>0){
											...
											$pay += $value["xprime"];
											$table .= '<tr>';
												$table .= '<td>'.$value["xacc"].'</td><td>'.$value["xaccdesc"].'</td><td>'.number_format($value["xprime"], $this->decimals, '.', '').'</td>';
											$table .= '</tr>';
										}
									}
							...
							$table .= '</tr>';
							$table .= '<tr>';
								$table .= '<td><strong>Cash In Hand :'.number_format($cashclbal, $this->decimals, '.', '').'</strong></td>';
							$table .= '</tr>';
							$table .= '<tr>';
								$table .= '<td><strong>Cash At Bank :'.$bankclbal.'</strong></td><td></><td><strong>'.number_format($totalcl, $this->decimals, '.', '').'</strong></td>';
							$table .= '</tr>';
							$table .= '</table>';
						$table .= '</td>';
					$table .= '</tr>';
					$table .= '<tr>';
					$table .= '<td><table width="100%"><tr align="right"><td>Total Receipt :</td><td></td><td>'.number_format(abs($rcpt), $this->decimals, '.', '').'</td></tr></table></td>';
					$table .= '<td><table width="100%"><tr align="right"><td>Total Payment :</td><td></td><td>'.number_format($pay, $this->decimals, '.', '').'</td></tr></table></td>';
					$table .= '</tr>';
					$table .= '<tr>';
					$rcpttotal = $rcpttotal+abs($rcpt);
					$table .= '<td><table width="100%"><tr align="right"><td>Total :</td><td></td><td>'.number_format(abs($rcpttotal), $this->decimals, '.', '').'</td></tr></table></td>';
					$table .= '<td><table width="100%"><tr align="right"><td>Total :</td><td></td><td>'.number_format($paytotal, $this->decimals, '.', '').'</td></tr></table></td>';
	...
	function accTable($fields, $row){
		...
		$table.='<tr>';
			$table.='<td></td><td><strong>Total</strong></td><td><strong>'.number_format($opdramttot, 2, '.', '').'</strong></td><td><strong>'.number_format(abs($opcramttot), 2, '.', '').'</strong></td><td><strong>'.number_format($opdramtcurtot, 2, '.', '').'</strong></td><td><strong>'.number_format(abs($opcramtcurtot), 2, '.', '').'</strong></td><td><strong>'.number_format($dramtcltot, 2, '.', '').'</strong></td><td><strong>'.number_format(abs($cramtcltot), 2, '.', '').'</strong></td>';
		$table.='</tr>';
		$table.='</tbody>';
		$table.='</table></div>'; 
		return $table;
	...
	function accPlTable($fields, $row, $rowexp){
		...
		$table.='<tr>';
			$table.='<td></td><td><strong>Total Income</strong></td><td><strong>'.number_format(abs($opamttot), $this->decimals, '.', '').'</strong></td><td><strong>'.number_format(abs($xcuramttot), $this->decimals, '.', '').'</strong></td><td><strong>'.number_format(abs($amtcltot), $this->decimals, '.', '').'</strong></td>';
		$table.='</tr>';
		$table.='<tr style="background-color:#ddd;">';
		...
		foreach($groupexp as $key=>$value){
			...
			foreach($value as $k=>$str){				
				if($k=="opamt" || $k=="xcuramt" || $k=="amtcl"){
					if($str<0)
						$table.='<td>('.htmlentities(number_format(abs($str), $this->decimals, '.', '')).')</td>';
					else
						$table.='<td>'.htmlentities(number_format($str, $this->decimals, '.', '')).'</td>';
					switch ($k){
						case "opamt":
							$expopamttot+=$str;
							break;
					...
					$table.='<td>'.htmlentities(number_format(floatval($str), $this->decimals, '.', '')).'</td>';
				}
		...
		$table.='<tr>';
			$table.='<td></td><td><strong>Profit & Loss</strong></td><td style="color:'.$opcolor.'"><strong>'.number_format($opprofit, $this->decimals, '.', '').'</strong></td><td style="color:'.$curcolor.'"><strong>'.number_format($curprofit, $this->decimals, '.', '').'</strong></td><td style="color:'.$clcolor.'"><strong>'.number_format($clprofit, $this->decimals, '.', '').'</strong></td>';
		$table.='</tr>';
		$table.='<tr>';
			$table.='<td></td><td><strong>Total Expenditure</strong></td><td><strong>'.number_format($expopamttot, $this->decimals, '.', '').'</strong></td><td><strong>'.number_format($expxcuramttot, $this->decimals, '.', '').'</strong></td><td><strong>'.number_format($expamtcltot, $this->decimals, '.', '').'</strong></td>';
		$table.='</tr>';
		$table.='</tbody>';
		$table.='</table></div>';
