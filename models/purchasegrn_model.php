...
class Purchasegrn_Model extends Model{
	...
	public function getgrnForConfirm($grnnum){
		...
		"xqtypur","xratepur","xqtypur*xratepur as ta","xqtypur*(xtaxrate*xratepur/100) as tt","xqtypur*xdisc as id" );
		//print_r($this->db->select("pabuziness", $fields));die;
	...
	public function getSinglePurchase($txnnum, $row){
		$fields = array("xrow",
		"xitemcode","xitemdesc",	
		"xqty","xgrnqty","xqty-xgrnqty as xbal","xratepur*xexch as xratepur","xwh");
	...
	public function getgrndt($txnnum){
		$fields = array("xrow",
		"xitemcode","xitemdesc","xstatus","xunitpur","xsup","xorg","xtotaltax","xtotaldisc",	
	...
	public function getGrnList($status){
		$fields = array("date_format(xdate, '%d-%m-%Y') as xdate","xgrnnum","xponum","xsup","(select xorg from sesup where sesup.bizid=pogrnmst.bizid and sesup.xsup=pogrnmst.xsup) as xsupname","xsupdoc");
		//print_r($this->db->select("pabuziness", $fields));die;
	...
	public function getrodqty($grnnum){
		$fields = array("COALESCE(sum(xqtypur),0) as xqty");
		
		$where = "bizid = ". Session::get('sbizid') ." and xgrnnum = '". $grnnum ."' and xcat='Rod'";	
		
		return $this->db->select("vgrndet", $fields, $where);
	}
	
	public function getcementqty($grnnum){
		$fields = array("COALESCE(sum(xqtypur),0) as xqty");
		
		$where = "bizid = ". Session::get('sbizid') ." and xgrnnum = '". $grnnum ."' and xcat='Cement'";	
