...
class Imreturn_Model extends Model{
	...
	public function getSingleQty($sl){
		$fields = array("xreqdeldetsl","xitemcode","(select xdesc from seitem where seitem.bizid=vdoreturndt.bizid and seitem.xitemcode=vdoreturndt.xitemcode) as xitemdesc","xqty-xreturnqty as xbal",
			"(select xstdprice from seitem where seitem.bizid=vdoreturndt.bizid and seitem.xitemcode=vdoreturndt.xitemcode) as xrate"
		);
		
		$where = "bizid = ". Session::get('sbizid') ." and xreqdeldetsl='".$sl."' LIMIT 1";	
		
		echo json_encode($this->db->select("vdoreturndt", $fields, $where));
	}
	...
	public function getreqdt($txnnum){
		$fields = array("xreqdeldetsl",
		"xitemcode","(select xdesc from seitem where seitem.bizid=vdoreturndt.bizid and seitem.xitemcode=vdoreturndt.xitemcode) as xitemdesc",	
		"xqty","xreturnqty");
		
		$where = " bizid = ". Session::get('sbizid') ." and xreqdelnum='".$txnnum."'";
		
		return $this->db->select("vdoreturndt", $fields, $where, " order by xreqdeldetsl");
	}

	public function getvreturndt($txnnum){
		$fields = array("*","(select xorg from secus where secus.bizid=vimreturndet.bizid and secus.xcus=vimreturndet.xcus) as xorg"
		,"(select xadd1 from secus where secus.bizid=vimreturndet.bizid and secus.xcus=vimreturndet.xcus) as xadd"
		,"(select xmobile from secus where secus.bizid=vimreturndet.bizid and secus.xcus=vimreturndet.xcus) as xmobile");
