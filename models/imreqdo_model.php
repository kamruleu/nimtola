...
class Imreqdo_Model extends Model{
	...
	public function getSingleQty($sonum,$sl){
		$fields = array("xrow","xitemcode","(select xdesc from seitem where seitem.bizid=vsalesdodt.bizid and seitem.xitemcode=vsalesdodt.xitemcode) as xitemdesc","xqty-xdoqty as xbal","xrate"
		);
		
		$where = "bizid = ". Session::get('sbizid') ." and xsonum='".$sonum."' and xrow='".$sl."' LIMIT 1";	
	...
	public function getreqdt($txnnum){
		...
		return $this->db->select("vsalesdodt", $fields, $where, " order by xrow");
	}
	...
	public function getvsalesdt($sonum){
		$fields = array("*","xqty-xdoqty as xbal",
		"(select xdesc from seitem where d.bizid=seitem.bizid and d.xitemcode=seitem.xitemcode) as xitemdesc",
		"(select xorg from secus where secus.bizid=d.bizid and secus.xcus=d.xcus) as xorg"
		,"(select xadd1 from secus where secus.bizid=d.bizid and secus.xcus=d.xcus) as xadd"
		,"(select xmobile from secus where secus.bizid=d.bizid and secus.xcus=d.xcus) as xmobile");
		//print_r($this->db->select("pabuziness", $fields));die;
		$where = " bizid = ". Session::get('sbizid') ." and xsonum = '". $sonum ."'";	
		return $this->db->select("vsalesdodt d", $fields, $where, " order by xrow");
	}
