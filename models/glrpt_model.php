...
class Glrpt_Model extends Model{
	...
	public function getjournal($vou){
		$fields = array("DATE_FORMAT(xdate,'%d-%m-%Y') as xdate","xrow","xbranch","xvoucher","xacc","xaccdesc","xaccsub","xaccsubdesc","xcheque","concat(xnarration,' ',COALESCE(xremarks,'')) as xnarration","(select if (xprime>0,xprime,'0')) as xprimedr", "(select if (xprime<0,abs(xprime),'0')) as xprimecr","xsite");
		//print_r($this->db->select("pabuziness", $fields));die;
		$where = "xrecflag='Live' and bizid = ". Session::get('sbizid') ." and xvoucher = '". $vou ."'";	
		//print_r($this->db->select("seitem", $fields, $where));die;
	...
	public function getledger($fdt, $tdt, $acc, $project=""){
		...
		$fields = array("DATE_FORMAT(xdate,'%d-%m-%Y') as xdate","xbranch","xvoucher","concat(xnarration,' ',COALESCE(xremarks,'')) as xnarration","(select if (xprime>0,xprime,'0')) as xprimedr", "(select if (xprime<0,abs(xprime),'0')) as xprimecr");
		//print_r($this->db->select("pabuziness", $fields));die;
	...
	public function getcashbankbook($fdt, $tdt, $type, $project=""){
		...
		$fields = array("DATE_FORMAT(xdate,'%d-%m-%Y') as xdate","xacc","xaccdesc","xbranch","xvoucher","concat(xnarration,' ',COALESCE(xremarks,'')) as xnarration","xprime");
		//print_r($this->db->select("pabuziness", $fields));die;
	...
	public function getsubledger($fdt, $tdt, $acc, $accsub, $project=""){
		...
		$fields = array("DATE_FORMAT(xdate,'%d-%m-%Y')","xbranch","xvoucher","concat(xnarration,' ',COALESCE(xremarks,'')) as xnarration","(select if (xprime>0,xprime,'0')) as xprimedr", "(select if (xprime<0,abs(xprime),'0')) as xprimecr");
		//print_r($this->db->select("pabuziness", $fields));die;
	...
	public function customertypebal($xcustype){
		$temp = "";
		if($xcustype!="")
			$temp = " and b.xcustype = '".$xcustype."'";
		
		$fields = array("a.xaccsub as xaccsub","b.xorg as xorg", "b.xcustype as xcustype","b.xadd1 as xadd1",
		"b.xphone as xmobile","a.xbal as xbal");
		$where = "a.bizid = b.bizid and a.xaccsub = b.xcus and b.bizid = ". Session::get('sbizid').$temp;
		return $this->db->select("(vcustomerbal a JOIN secus b)", $fields, $where);
	}

	

	public function salesmanwisebal($xagent, $fdate){
		$temp = "";
		if($xagent!="")
			$temp = " and (SELECT xagent from secus where secus.bizid = gldetailview.bizid and secus.xcus = gldetailview.xaccsub)='".$xagent."'";
		
		$fields = array("`gldetailview`.`bizid` AS `bizid`","`gldetailview`.`xaccsub` AS `xaccsub`","(SELECT xorg from secus where secus.bizid = gldetailview.bizid and secus.xcus = gldetailview.xaccsub) as xorg", "(SELECT xadd1 from secus where secus.bizid = gldetailview.bizid and secus.xcus = gldetailview.xaccsub) as xadd1", "(SELECT xphone from secus where secus.bizid = gldetailview.bizid and secus.xcus = gldetailview.xaccsub) as xmobile", "(SELECT xagent from secus where secus.bizid = gldetailview.bizid and secus.xcus = gldetailview.xaccsub) as xagent", "round(sum(`gldetailview`.`xprime`),2) AS `xbal`");
		$where = "(`gldetailview`.`xaccsource` = 'Customer') and xdate<= '".$fdate."' and bizid = ". Session::get('sbizid').$temp;
		return $this->db->select("gldetailview", $fields, $where, " group by `gldetailview`.`bizid`,`gldetailview`.`xaccsub`");
	}
